...
class Transformer:
    ...
    @staticmethod
    def transforming_demographics(demographics):
        """
        Transform demographics dataset grouping by state an calculate all the totals and ratios for every race
         in every state.
        returns transformed demographics dataset
        """
        demo = demographics.groupBy(col("State Code").alias("State_code"), col("State")).agg(
    ...
    @staticmethod
    def transforming_immigrants(immigrants):
        """
        ...
        returns immigration dataset transformed
        """
        immigrants = immigrants.withColumn("arrival_date-split", split(col("arrival_date"), "- ")).withColumn("arrival_year",col("arrival_date-split")[0]).withColumn("arrival_month", col("arrival_date-split")[1]).withColumn("arrival_day", col("arrival_date-split")[2]).drop("arrival_date-split")
